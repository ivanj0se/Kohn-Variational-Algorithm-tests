...
def build_matrices(Energy, N_L=30, gamma=1.5, R_max=40.0):
    """
    Gaussian quadrature with analytical derivatives.

    Parameters:
        Energy : float — scattering energy E = ħ²k²/(2μ)
        N_L    : int   — number of basis functions is (N_L - 1); increase for
                         better convergence / unitarity of S-matrix
        gamma  : float — exponential decay parameter shared by the basis
                         functions and the cutoff factor; controls how
                         quickly the basis damps at large R
        R_max  : float — upper integration limit for the quadrature;
                         must be large enough that the integrands have
                         decayed, but not so large that oscillatory
                         tails cause numerical noise
